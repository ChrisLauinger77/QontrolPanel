...
- Extract new translation messages explicitly with `cmake --build build --target update_translations`, then review the `.ts` changes; normal builds only compile the existing catalogs.
