...
The settings window uses the documented DWM transient system backdrop with activation-aware presentation. The DWM material covers the framed window bounds without extending the native frame through Qt's client area. The Qt window remains transparent from creation: while focused, its QML background exposes the live desktop hue; while unfocused, QML paints the neutral panel surface used by Windows for inactive settings windows. Activation changes recheck High Contrast, DWM composition, the Windows transparency-effects setting, and the native result so unavailable effects or an enable-time failure retain the opaque fallback. Its setting cards use a single translucent layer fill so they remain distinct without hiding the material.
