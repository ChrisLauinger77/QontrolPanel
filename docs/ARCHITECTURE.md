...
The settings window requests the documented DWM main-window system backdrop (Mica) for its native title bar, matching the material intended for long-lived Windows settings surfaces. Qt Quick renders the decorated window's client area through an opaque swapchain, so QML paints the neutral panel surface in both activation states instead of exposing transparent pixels that resolve to white. Activation changes recheck High Contrast, DWM composition, the Windows transparency-effects setting, and the native result so unavailable effects retain the standard opaque title-bar fallback. Its setting cards use a single translucent layer fill so they remain distinct against the Windows Settings-style client surface.
