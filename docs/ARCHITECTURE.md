...
The settings window requests the documented DWM main-window system backdrop (Mica), matching the material intended for long-lived Windows settings surfaces. Qt's `ExpandedClientAreaHint` and `NoTitleBarBackgroundHint` extend the Qt Quick surface beneath the system caption controls while `ApplicationWindow`'s automatic safe-area padding keeps settings content below them. `DWMWA_REDIRECTIONBITMAP_ALPHA` lets DWM use the surface's premultiplied client alpha instead of resolving transparent pixels to white. While focused, the transparent QML background exposes Mica across the client area; while unfocused, QML paints the neutral panel surface. Activation changes recheck High Contrast, DWM composition, the Windows transparency-effects setting, the redirection-alpha attribute, and the native backdrop result so any unavailable effect retains the opaque fallback. Its setting cards use a single translucent layer fill so they remain distinct against the Mica base layer.
