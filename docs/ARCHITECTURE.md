...
The settings window uses the documented DWM main-window system backdrop (Mica) with activation-aware presentation, matching the material intended for long-lived Windows settings surfaces. The DWM material covers the framed window bounds without extending the native frame through Qt's client area. The Qt window remains transparent from creation: while focused, its QML background exposes the subdued Mica wallpaper tint; while unfocused, QML paints the neutral panel surface used by Windows for inactive settings windows. Activation changes recheck High Contrast, DWM composition, the Windows transparency-effects setting, and the native result so unavailable effects or an enable-time failure retain the opaque fallback. Its setting cards use a single translucent layer fill so they remain distinct without hiding the material.
