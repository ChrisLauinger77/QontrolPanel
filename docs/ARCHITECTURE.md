...
`WindowsBackdrop` owns the native material lifecycle for the main controls panel, its separate media card, `MediaOverlay`, and `ChatMixNotification`. After the Qt event loop starts, it creates the required `Windows.System.DispatcherQueue` on the UI thread, shares it across all backdrop surfaces, and shuts it down while continuing to pump window messages. Each exact-size Qt window receives its own Windows App SDK `DesktopAcrylicController`, composition target, and `SystemBackdropConfiguration`. The controller uses the Windows thin-acrylic variant with a transparent tint so the backdrop hue comes from the wallpaper or window behind it. Its neutral luminosity and fallback colors follow the current light/dark theme, and it keeps these tray surfaces input-active from controller creation onward because overlays and notifications intentionally do not activate like normal application windows.
